...
%%% @doc
%%% Fake TLS 'CBC' stream codec with REALITY-style authentication
...
%% REALITY ساختار فیلد Random در
%% Bytes 0-10:   Random data (دست‌نخورده)
%% Byte  11:     Magic byte (0xA7)
%% Byte  12:     Version (1)
%% Bytes 13-16:  Timestamp (4 bytes, big-endian)
%% Bytes 17-31:  Random padding (15 bytes)
-define(REALITY_MAGIC_OFFSET, 11).
-define(REALITY_MAGIC_BYTE, 16#A7).
-define(REALITY_VERSION_OFFSET, 12).
-define(REALITY_VERSION, 1).
...
%% ============================================================================
%% Helper: create zero binary of given size
%% ============================================================================
zero_binary(Size) ->
    <<0:Size/unit:8>>.
...
%% Extract REALITY auth data from XOR result
-spec reality_extract_auth(binary()) -> {ok, non_neg_integer()} | {error, term()}.
...
%% Verify REALITY HMAC and timestamp
-spec reality_verify_hmac(binary(), binary(), binary()) -> 
    {ok, non_neg_integer()} | {error, term()}.
reality_verify_hmac(Data, Random, Secret) ->
    ReceivedHMAC = Random,
    
    <<Left:?DIGEST_POS/binary, _:?DIGEST_LEN/binary, Right/binary>> = Data,
    
    Zeros = zero_binary(?DIGEST_LEN),
    ExpectedHMAC = hmac(sha256, Secret, [Left, Zeros, Right]),
...
%% Create REALITY Random field
-spec reality_create_random(binary(), binary(), non_neg_integer()) -> binary().
reality_create_random(Data, Secret, Timestamp) ->
    <<Left:?DIGEST_POS/binary, _:?DIGEST_LEN/binary, Right/binary>> = Data,
    
    %% Build REALITY structure
    PadBeforeMagic = ?REALITY_MAGIC_OFFSET,
    PadAfterTimestamp = ?DIGEST_LEN - ?REALITY_TIMESTAMP_OFFSET - 4,
    
    RealityData = <<0:PadBeforeMagic/unit:8,
                    ?REALITY_MAGIC_BYTE:8,
                    ?REALITY_VERSION:8,
                    Timestamp:32/unsigned-big,
                    0:PadAfterTimestamp/unit:8>>,
    
    Zeros = zero_binary(?DIGEST_LEN),
    Digest = hmac(sha256, Secret, [Left, Zeros, Right]),
...
from_client_hello(Data, Secret, AllowedDomains) ->
    ...
    SrvHello0 = make_srv_hello(zero_binary(?DIGEST_LEN), SessionId, KeyShare),
...
make_server_digest(<<Left:?DIGEST_POS/binary, _:?DIGEST_LEN/binary, Right/binary>>, Secret) ->
    Msg = [Left, zero_binary(?DIGEST_LEN), Right],
...
make_client_hello(Timestamp, SessionId, Secret, SniDomain) when byte_size(SessionId) == 32,
    ...
    FakeRandom0 = zero_binary(?DIGEST_LEN),
    Hello0 = Pack(FakeRandom0),
    
    %% REALITY: Create authentic Random field
