...
%%% Enhanced with post-CCS Certificate/CertificateVerify/Finished records
%%% for DPI evasion while maintaining MTProto hash compatibility
%%% @end
...
-record(st, {
    post_ccs_padding :: binary() | undefined
}).
...
%% ============================================================================
%% @doc Build fake Certificate record (TLS 1.3 - Certificate message)
%% struct {
...
%% @end
%% ============================================================================
-spec build_fake_certificate(map()) -> binary().
build_fake_certificate(#{cert_chain_size := {MinSize, MaxSize}}) ->
    CertSize = MinSize + rand:uniform(MaxSize - MinSize),
    FakeCert = crypto:strong_rand_bytes(CertSize),
    
    %% Certificate message: tag(11) + length(3) + request_context(1 byte: 0) + cert_list_length(3) + cert_data
...
%% ============================================================================
%% @doc Build fake CertificateVerify record (TLS 1.3)
%% struct {
...
%% @end
%% ============================================================================
-spec build_fake_cert_verify(map()) -> binary().
build_fake_cert_verify(#{cert_verify_algo := rsa_pss_rsae_sha256}) ->
    ...
    Algorithm = <<16#08, 16#04>>,  %% rsa_pss_rsae_sha256
...
build_fake_cert_verify(#{cert_verify_algo := ecdsa_secp256r1_sha256}) ->
    ...
    Algorithm = <<16#04, 16#03>>,  %% ecdsa_secp256r1_sha256
    Payload = <<Algorithm/binary, SigSize:?u16, FakeSig/binary>>,
    PayloadSize = byte_size(Payload),
    <<?TLS_TAG_CERT_VERIFY, PayloadSize:?u24, Payload/binary>>;
...
%% ============================================================================
%% @doc Build fake Finished record (TLS 1.3)
%% struct {
%%     opaque verify_data[Hash.length];
%% } Finished;
%% @end
%% ============================================================================
-spec build_fake_finished(map()) -> binary().
...
%% ============================================================================
%% @doc Build post-CCS padding to randomize encrypted traffic size
%% @end
%% ============================================================================
-spec build_post_ccs_padding(map()) -> binary().
build_post_ccs_padding(#{post_ccs_padding := {MinSize, MaxSize}}) ->
    PadSize = MinSize + rand:uniform(MaxSize - MinSize),
    crypto:strong_rand_bytes(PadSize);
build_post_ccs_padding(_) ->
    crypto:strong_rand_bytes(512).

%% ============================================================================
%% @doc Parse fake-TLS "ClientHello" and generate complete handshake response.
%% 
%% CRITICAL DESIGN:
%% - Response0 (used for MTProto hash) = SrvHello + CCS + Data ONLY
%% - Post-CCS records (Certificate, CertVerify, Finished) come AFTER Data
%% - They are encrypted (post-CCS) but do NOT affect MTProto hash
%% - DPI sees a complete TLS 1.3 handshake
%% @end
%% ============================================================================
...
from_client_hello(Data, Secret, AllowedDomains) ->
    ...
    %% Detect CompressCertificate capability
    ...
    %% Pick a random profile for post-CCS record sizes
    ...
    %% ============================================================
    %% Build ServerHello (advertises compress_certificate if supported)
    %% ============================================================
    ...
    %% ============================================================
    %% Build MTProto hash input: ONLY SrvHello + CCS + Data
    %% This is CRITICAL - must match exactly what Telegram X expects
    %% ============================================================
    ...
    %% ============================================================
    %% Build post-CCS records (encrypted, NOT part of MTProto hash)
    %% These make DPI see a complete TLS 1.3 handshake
    %% ============================================================
    ...
    %% Random padding to vary encrypted data size
    Padding = build_post_ccs_padding(Profile),
    PaddingFrame = as_tls_frame(?TLS_REC_DATA, Padding),
    
    %% ============================================================
    %% Final Response:
    %% [SrvHello (clear)] + [CCS (clear)] + [Data (encrypted)] + 
    %% [Certificate (encrypted)] + [CertVerify (encrypted)] + 
    %% [Finished (encrypted)] + [Padding (encrypted)]
    %%
    %% MTProto hash covers: SrvHello + CCS + Data
    %% DPI sees: complete TLS 1.3 handshake
    %% ============================================================
    Response = [SrvHelloFrame, 
                ChangeCipherFrame, 
                DataFrame, 
                CertFrame, 
                CertVerifyFrame, 
    ...
    St = #st{post_ccs_padding = Padding},
    {ok, Response, Meta, St}.
...
make_srv_hello(Digest, SessionId, {KeyShareGroup, KeyShareKey}, CompCertAlgo) ->
    ...
    Extensions = case CompCertAlgo of
        brotli -> ExtensionsBase ++ [<<16#00, 16#1b, 16#00, 16#03, 16#02, 16#02, 16#00>>];
        _ -> ExtensionsBase
    end,
...
%% ============================================================================
%% @doc Parses "ServerHello" response.
%% Updated to handle Certificate, CertificateVerify, and Finished records.
%% @end
%% ============================================================================
...
parse_server_hello(<<?TLS_REC_HANDSHAKE, ?TLS_12_VERSION, HSLen:?u16, Handshake:HSLen/binary,
                     ?TLS_REC_CHANGE_CIPHER, ?TLS_12_VERSION, CCLen:?u16, ChangeCipher:CCLen/binary,
                     ?TLS_REC_DATA, ?TLS_12_VERSION, DLen:?u16, Data:DLen/binary,
                     ?TLS_REC_HANDSHAKE, ?TLS_12_VERSION, CertLen:?u16, _Cert:CertLen/binary,
                     ?TLS_REC_HANDSHAKE, ?TLS_12_VERSION, CertVerifyLen:?u16, _CV:CertVerifyLen/binary,
                     ?TLS_REC_HANDSHAKE, ?TLS_12_VERSION, FinishedLen:?u16, _Fin:FinishedLen/binary,
                     Tail/binary>>) ->
    {Handshake, ChangeCipher, Data, Tail};
parse_server_hello(<<?TLS_REC_HANDSHAKE, ?TLS_12_VERSION, HSLen:?u16, Handshake:HSLen/binary,
                     ?TLS_REC_CHANGE_CIPHER, ?TLS_12_VERSION, CCLen:?u16, ChangeCipher:CCLen/binary,
                     ...
                     ?TLS_REC_HANDSHAKE, ?TLS_12_VERSION, CertVerifyLen:?u16, _CV:CertVerifyLen/binary,
                     ?TLS_REC_HANDSHAKE, ?TLS_12_VERSION, FinishedLen:?u16, _Fin:FinishedLen/binary,
                     ?TLS_REC_DATA, ?TLS_12_VERSION, PadLen:?u16, _Pad:PadLen/binary,
                     Tail/binary>>) ->
    {Handshake, ChangeCipher, Data, Tail};
